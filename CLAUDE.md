...
**Native (iOS/Android):** `app/_layout.tsx` → Stack (with `LocaleContext.Provider`) → `app/(tabs)/_layout.tsx` → 4-tab navigator (5 tabs for staff) → screens in `app/(tabs)/`.  
...
- `@react-native-community/datetimepicker` — date/time picker used in create-class form

**Planned:**
- `expo-notifications` — push notifications for payment flow
- Green Invoice API calls via Supabase Edge Function (never on device)
...
| 4 — Wire Classes screen | `feature/classes-live` | Merged to master |
| 5 — Booking flow | `feature/bookings` | Merged to master |
| 6 — Instructor & owner views | `feature/staff-views` | In progress |
...
### Completed — Step 5: Booking flow (`feature/bookings`)

- `app/class/[id].tsx` — class detail + Book/Cancel button
- `src/hooks/useMyBookings.ts` + `app/(tabs)/bookings.tsx` — live bookings list
- Atomic `book_class` + `cancel_booking` Postgres functions (capacity check, waitlist promotion)
- Locale switching removed; all dates in Hebrew (`he-IL`), all times 24 h; scrollable RTL date strip

### Completed — Step 6: Instructor & owner views (`feature/staff-views`)

- Conditional **Manage** tab — visible to `owner` and `instructor` only (`href: null` for clients)
- `app/(tabs)/manage.tsx` — full 14-day class list; instructor colour badges; instructor filter bar; cancel-class action
- `app/roster/[classId].tsx` — participant list with per-client attendance checkboxes
- `app/class/new.tsx` — create-class form (title, category, date/time, duration, capacity, location); sets `instructor_id = auth.uid()`
- `supabase/migrations/0007_staff_functions.sql` — `get_managed_classes`, `get_class_roster`, `mark_attendance`
- Both owner and instructor can see all classes, create classes, and view/mark rosters
- Payment flow distinction (Step 7): notification goes to whoever's `instructor_id` is on the finished class

### Next — Step 7: Push notifications + post-lesson payment flow (`feature/payments`)

**Goal:** When a class ends, the instructor who taught it receives a push notification that opens a per-client payment-action screen. Clients whose lessons are marked for payment get their own notification with an in-app payment link.

**Sub-tasks:**

1. **`expo-notifications` setup**
   - Install `expo-notifications`; request permission on login
   - Store device push token in `profiles.push_token` (migration `0008_push_tokens.sql`)
   - Register/update token each time the user logs in

2. **Trigger: lesson-end detection**
   - Supabase Edge Function (`functions/notify-lesson-end/index.ts`) on a pg_cron job: every 5 min, find classes whose `scheduled_at + duration_minutes` is in the past and that haven't yet had payments processed
   - Send push to the class's `instructor_id` → deep-link to `/payment-review/[classId]`

3. **Instructor payment-review screen** (`app/payment-review/[classId].tsx`)
   - Lists all confirmed attendees (reuses `get_class_roster`)
   - Per client, three action buttons: **Cash** / **Waived** / **Send payment request**
   - Cash/Waived → insert `payments` row with matching `method`
   - "Send payment request" → insert `payments` row with `method = 'pending'` + send push to client → deep-link to `/pay/[bookingId]`

4. **Client payment screen** (`app/pay/[bookingId].tsx`)
   - Owner-taught lessons: embed Green Invoice payment page in a WebView
   - Instructor-taught lessons: show "your instructor will collect payment separately"
   - On successful payment: Green Invoice webhook → Edge Function → update `payments.method = 'paid'`

5. **Green Invoice integration** (`src/lib/greenInvoice.ts` + Edge Function)
   - Owner credentials stored as Supabase secrets (never on device)
   - Edge Function creates a payment link via Green Invoice API and returns URL to client
   - Webhook receiver updates `payments` row

6. **Migration** `0008_push_tokens.sql` — add `push_token text` column to `profiles`

After that: Step 8 (i18n polish — Hebrew translations audit, RTL check on all new screens, auto-detect device locale).
